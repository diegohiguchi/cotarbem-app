angular.module('ng-walkthrough', [])
    .directive("walkthrough", ['$log', '$timeout', '$window', '$injector', '$compile',
        function ($log, $timeout, $window, $injector, $compile) {
            ...
            var BUTTON_CAPTION_DONE = "Pular";
            var BUTTON_CAPTION_NEXT = "Próximo";
            var PADDING_HOLE = 5;
            ...
            var template = [
                ...
                '{{buttonCaption}}',
                '<button class="walkthrough-element walkthrough-button-positive walkthrough-next-button" type="button" ng-if="useButtonNext" ng-click="onClick($event)">',
                '{{buttonCaptionNext}}',
                '</button>',
                '</div>',
                '</div>',
                ...
                link: function (scope, element, attrs, ctrl, $transclude) {
                    ...
                    var attemptTouchEvent = function (e) {
                        if (scope.clickEvent === 'touch' && canTouch) { //We need this in case both angular and ionic are for some reason loaded
                            if ((!scope.useButton && !scope.useButtonNext) ||
                    ...
                    var init = function (scope) {
                        ...
                        scope.onClick = function ($event) {
                            var posicao = parseInt(scope.focusElementSelector.split('-')[1]) + 1;
                            var id = scope.focusElementSelector.split('-')[0] + '-' + posicao;
                            scope.focusElementSelector = id;

                            switch (posicao) {
                                case 1:
                                    scope.mainCaption = "Bem vindo ao Cotar Bem! Cliente crie aqui a sua cotação.";
                                    scope.setFocusOnElement(id);
                                    break;
                                case 2:
                                    scope.mainCaption = "Fornecedor adicione o segmento que você trabalha e aguarde as solicitações para cotação.";
                                    scope.setFocusOnElement(id);
                                    scope.forceCaptionLocation = 'TOP';
                                    break;
                                case 3:
                                    scope.mainCaption = "Seu histórico de pedidos de compras.";
                                    scope.setFocusOnElement(id);
                                    scope.forceCaptionLocation = 'BOTTOM';
                                    break;
                                case 4:
                                    scope.mainCaption = "Você será notificado quando houver abertura de cotações, encerramento e mensagem do chat.";
                                    scope.setFocusOnElement(id);
                                    scope.forceCaptionLocation = 'BOTTOM';
                                    break;
                                case 5:
                                    scope.mainCaption = "Chat para tirar dúvidas com o fornecedor, após encerrada a cotação.";
                                    scope.setFocusOnElement(id);
                                    scope.forceCaptionLocation = 'TOP';
                                    break;
                                case 6:
                                    scope.mainCaption = "Seu perfil e endereço.";
                                    scope.setFocusOnElement(id);
                                    scope.forceCaptionLocation = 'TOP';
                                    scope.useButton = false;
                                    scope.buttonCaptionNext = 'Fechar';
                                    break;
                                case 7:
                                    scope.closeWalkthrough();
                                    break;
                        ...
                        scope.buttonCaptionNext = BUTTON_CAPTION_NEXT;
